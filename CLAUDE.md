...
- **Partner extra-work**: `/p/` has a "¿Hace falta más trabajo?" box → `POST /api/job/<token>` `action:'suggest_extra'` (text only, never a price) → logged as `partner_suggested_extra` → gold banner in the admin order detail (Mike calls the client, re-quotes, adds it). Partner never sets prices or deals directly with the client.
